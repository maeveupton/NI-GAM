plot_data<-function(SL_df,save_name,n_sites)
{
  ...
  plot_data <- ggplot() +
    ...
    geom_point(data = SL_df_proxy, aes(y = RSL, x = Age*1000,colour="black"), size = 0.2) +
    xlab("Year (CE)") +
  ...
  SL_df_TG_50 <- SL_df_TG %>% 
    filter(Age_range >= 50)
  # Only plotting sites with more than 100 years data----
  plot_TG_data_50 <- ggplot(SL_df_TG_50) +
    geom_rect(aes(
      xmin = Age*1000 - Age_er_min*1000, xmax = Age*1000 + Age_er_max*1000,
      ymin = RSL - RSL_er_min, ymax = RSL + RSL_er_max,
      fill = "grey",
  ...
  ggsave(plot_TG_data_50, file = "fig/TG_plot_1_over_50yrs.pdf", width = 10, height = 6)
