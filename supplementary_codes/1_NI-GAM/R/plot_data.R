plot_data<-function(SL_df,save_name,n_sites)
{
  ...
  all_data_sites<-factor(paste0(SL_df_TG$Longitude, SL_df_TG$Longitude), labels = 1:55)
  ...
  plot_TG_data <- ggplot() +
    # geom_rect(data = SL_df_TG, aes(
    #   #xmin = Age*1000 - Age_er_min*1000, xmax = Age*1000 + Age_er_max*1000,
    #   xmin = Age*1000 - Age_er_average*1000, xmax = Age*1000 + Age_er_average*1000,
    #   ymin = RSL - RSL_er_min, ymax = RSL + RSL_er_max,
    #   fill = "1 sigma sea level and age uncertainty",
    # ), alpha = 0.5) +
    
    geom_line(data = SL_df_TG, aes(y = RSL, x = Age*1000,
                                   colour = SiteName), size = 0.3) +
    xlab("Year (CE)") +
    ylab("RSL (m)") +
    theme_bw()+
    labs(colour = "")+
  ...
  plot_TG_data
  ggsave(plot_TG_data, file = "fig/TG_plot.pdf", width = 10, height = 6)
...
plot_map<-function(SL_df,save_name)
{
  ...
  #   theme_classic()+
  #   coord_sf(xlim = c(-170, -38), ylim = c(23, 85), expand = FALSE)+
  #   geom_richtext(x = -100, y =65,label = "North America")+
  #   geom_rect(aes(xmin =-85, xmax = -50, ymin = 24, ymax = 53), color = "black", fill = NA)  +
  ...
  NAmap_new <- 
    ...
    #geom_label_repel(data=sites,aes(x=Longitude,y=Latitude,label = SiteName), max.overlaps = 50,fontface = "bold", size = 3)+
    # geom_label_repel(data=sites_4,aes(x=Longitude,y=Latitude,label = SiteName),
    #                  max.overlaps = 6,fontface = "bold", size = 3)+
