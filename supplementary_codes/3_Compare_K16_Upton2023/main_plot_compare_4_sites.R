...
#erica_file <- "50_year_pred"
upton_file <- "Upton_result_17Nov"


#---Read in K16 results for full model fit----
K16_full_model_fit <- read.table(file = paste0("data/",erica_file,"/RSLsFull.tsv"),sep = '\t',check.names = FALSE)#50 year pred
...
K16_full_model_fit_df <- K16_full_model_fit_df %>% 
  group_by(SiteName) %>% 
  mutate(RSL_er_up = RSL + qnorm(0.975)*sd_RSL,#/sqrt(no_rows),
...
NIGAM_full_model <- read.csv(paste0("data/",upton_file,"/total_model_fit.csv"))
NIGAM_full_model$ID <- "This study"
# Match the site names
NIGAM_full_model$SiteName <- gsub("\\,.*","",NIGAM_full_model$SiteName)
NIGAM_full_model <- NIGAM_full_model %>% filter(SiteName %in% c("East River Marsh",
                       "Little Manatee River",
                       "Big River Marsh",
                       "Cedar Island")) 
#---Full Model Fit Plot----
full_model_fit_plot <- ggplot()+
  ...
  theme_bw()
full_model_fit_plot
#ggsave(full_model_fit_plot, file = "fig/total_model_compare_4sites_4sites.pdf", width = 10, height = 6)
ggsave(full_model_fit_plot, file = paste0("fig/",erica_file,"/total_model_compare_4sites_4sites.pdf"), width = 10, height = 6)
#ggsave(full_model_fit_plot, file = "fig/ice5_refyr0/total_model_compare_4sites_4sites.pdf", width = 10, height = 6)
...
#K16_common_reg <- read.table(file = "data/ice5_refyr0/RSLsCommon Regional.tsv",sep = '\t',check.names = FALSE)# 50 year Pred
...
K16_common_reg_wide_df<- K16_common_reg_long_df %>% 
  pivot_wider(names_from = RSL_class, values_from = RSL)
#----Clean data----
K16_common_df_old <- K16_common_reg_wide_df %>%
  rename(RSL=RSLs, sd_RSL = SDs, 
         Longitude=long,Latitude= lat,SiteName = Site) %>% 
  mutate(RSL = RSL/1000,#m
         sd_RSL = sd_RSL/1000,
  ...
  group_by(SiteName) %>% 
  mutate(no_rows = n())
K16_common_df <- K16_common_df_old %>% 
  group_by(SiteName) %>% 
  # mutate(RSL_er_up = RSL + qnorm(0.975)*sd_RSL/sqrt(no_rows),
  #        RSL_er_lwr = RSL - qnorm(0.975)*sd_RSL/sqrt(no_rows)) %>% 
  mutate(RSL_er_up = RSL + qnorm(0.975)*sd_RSL,
         RSL_er_lwr = RSL - qnorm(0.975)*sd_RSL) %>% 
  
  filter(SiteName %in% c("East River Marsh", "Little Manatee River","Big River Marsh", "Cedar Island")) 

#--- Upton Regional Fit data----
NIGAM_regional <- read.csv(paste0("data/",upton_file,"/regional_grid_model_fit.csv"))
...
com_reg_plot <- ggplot()+
  ...
  theme_bw()
com_reg_plot
ggsave(com_reg_plot, file = paste0("fig/",erica_file,"/regional_compare_4sites_4sites.pdf"), width = 10, height = 6)
#ggsave(com_reg_plot, file = "fig/regional_compare_4sites.pdf", width = 10, height = 6)
#ggsave(com_reg_plot, file = "fig/ice5_refyr0/regional_compare_4sites_4sites.pdf", width = 10, height = 6)
...
K16_lin_GIA_df <- K16_lin_GIA_df %>% 
  group_by(SiteName) %>% 
  mutate(RSL_er_up = RSL + qnorm(0.975)*sd_RSL,#/sqrt(no_rows),
         RSL_er_lwr = RSL - qnorm(0.975)*sd_RSL) %>% filter(SiteName %in% c("East River Marsh",
                                                                            "Little Manatee River",
                                                                            "Big River Marsh",
                                                                            "Cedar Island")) 
...
# CHECK This one
NIGAM_GIA <- read.csv(paste0("data/",upton_file,"/gia_yoffset_model_fit.csv"))
NIGAM_GIA$ID <- "This study"
# Match the site names
NIGAM_GIA$SiteName <- gsub("\\,.*","",NIGAM_GIA$SiteName)

NIGAM_GIA <- NIGAM_GIA %>% filter(SiteName %in% c("East River Marsh",
                                             "Little Manatee River",
                                             "Big River Marsh",
                                             "Cedar Island")) 
#---Linear GIA Plot----
lin_GIA_plot <- ggplot()+
  ...
  theme_bw()
lin_GIA_plot
#ggsave(lin_GIA_plot, file = "fig/GIA_compare_4sites.pdf", width = 10, height = 6)
ggsave(lin_GIA_plot, file = paste0("fig/",erica_file,"/GIA_compare_4sites.pdf"), width = 10, height = 6)
#ggsave(lin_GIA_plot, file = "fig/ice5_refyr0/GIA_compare_4sites.pdf", width = 10, height = 6)
...
#K16_local <- read.table(file = "data/ice5_refyr0/RSLsLocal Non-linear.tsv",sep = '\t',check.names = FALSE)
...
NIGAM_local <- read.csv(paste0("data/",upton_file,"/local_model_fit.csv"))
NIGAM_local$ID <- "This study"
# Match the site names
NIGAM_local$SiteName <- gsub("\\,.*","",NIGAM_local$SiteName)

NIGAM_local<- NIGAM_local %>% filter(SiteName %in% c("East River Marsh",
                                                     "Little Manatee River",
                                                     "Big River Marsh",
                                                     "Cedar Island")) 
#---Local Plot----
local_plot <- ggplot()+
  ...
  theme_bw()
local_plot
#ggsave(local_plot, file = "fig/local_compare_4sites.pdf", width = 10, height = 6)
ggsave(local_plot, file = paste0("fig/",erica_file,"/local_compare_4sites.pdf"), width = 10, height = 6)
#ggsave(local_plot, file = "fig/ice5_refyr0/local_compare_4sites.pdf", width = 10, height = 6)
